...
### Unit Tests — Vitest
Unit tests are added *only* where pure logic earns them — not for coverage targets. Examples: filename derivation, card HTML/markdown handling. Most features rely on E2E.
...
### Live Smoke Test
A single test uploads a one-image PDF and runs the *real* pipeline (real ai-server + OpenAI), asserting cards come back. This is the only automated path that spends money, and it's cheap (one image, ~10 seconds, pennies). It runs on push-to-`main` and on manual trigger, not on every PR.
